...
- `packages/audio` — pure, Node-testable audio core: a `SharedArrayBuffer` ring buffer (`createRingSab`/`attachRing`/`push`/`pull`/`waitFor`) for the AudioWorklet-to-ASR-worker hot path, a silence gate (`rms`/`isSilent`), the `TranscriptionEngine` contract plus a deterministic `fakeEngine` test double, greedy CTC decode (`ctcGreedy`), and the consuming loop (`runAsrLoop`, reports `rtf`/`droppedFrames`/`windows`); zero runtime dependencies. AudioWorklet, Worker, the real ONNX engine, and OPFS land in later slices. See [packages/audio/README.md](./packages/audio/README.md).
