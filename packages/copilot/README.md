...
3. `rascunho`: `APROVAR` → `aprovado` (final); `DESCARTAR` → `descartado` (final, descarte direto sem passar por aviso); `AVISO_VENCIMENTO` → `aVencer`, gravando `avisoEmitidoEm`.
...
- `separarPorAncora(afirmacoes)` — mantém só afirmações com `ancoras.length > 0`; devolve `{ comAncora, descartadas }`. Usa o mesmo formato de âncora `{ inicioMs, fimMs }` já estabelecido em `packages/diarization` (`PalavraAsr`/`PalavraAtribuida`).
