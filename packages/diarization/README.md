...
- `processarDiarizacao(palavras: readonly PalavraAsr[], turnos: readonly TurnoLocutor[], cadastrado: readonly number[], candidatos: readonly LocutorCandidato[], margemMinima?: number): readonly TrechoCanonico[]` — composição pura das três funções acima, mesma ordem (`atribuirLocutores` → `classificarLocutores` → `montarTranscricaoCanonica`), sem lógica própria além do encadeamento.
...
**`processarDiarizacao` não repete o literal `0.05` de `margemMinima`:** o parâmetro é opcional e reencaminhado tal e qual para `classificarLocutores`, cujo próprio parâmetro por omissão já se aplica quando recebe `undefined` — o default do produto continua a viver num sítio só (`classify.ts`), evitando que os dois valores um dia divirjam.
