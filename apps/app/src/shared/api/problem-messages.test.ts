...
describe('translateProblemCode — answers every code the backend declares', () => {
  ...
  it.each([...problemCodes])('answers %s with its own message', (code) => {
    const i18nInstance = setupI18n({
      locale: 'pt-BR',
      messages: { 'pt-BR': { 'errors.generic': GENERIC_SENTINEL } },
    })

    const result = translateProblemCode(code, {}, i18nInstance)

    expect(result).not.toBe(GENERIC_SENTINEL)
