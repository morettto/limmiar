...
**S11-04**: `problem-codes.ts` é gerado (`npm run generate:problem-codes`, a partir de `apps/api/src/Api/**/*ProblemCodes.cs`) -- rodar o gerador sempre que a API ganhar um código novo, senão `Record<ProblemCode, MessageDescriptor>` em `problem-messages.ts` para de ser exaustivo e `tsc -b` falha (TS2740). `auth.forbidden`, `key_pair.not_found`, `key_pair.public_key_conflict`, `link.invite_not_found`, `link.already_linked`, `link.not_authorized` e `link.not_found` entraram assim, cada um com mensagem própria -- nenhum reusa o texto genérico de `errors.generic` deliberadamente, mesmo quando dois motivos diferentes colapsam no mesmo código (ex.: `link.not_authorized` cobre "não é profissional ativa" e "não é paciente").

Nota à parte: a secção acima (`putJson`/`getJson`/`deleteRequest`/`../devices/base64.ts`) já não corresponde ao módulo atual (`client.ts` exporta só `request()`; bytes passam por `../lib/base64.ts`) -- drift anterior a este ticket, fora do âmbito do S11-04.
