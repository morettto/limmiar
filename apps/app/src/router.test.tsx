...
vi.mock('./auth/AuthScreen', () => ({ AuthScreen: vi.fn(() => <div data-testid="auth-screen" />) }))
vi.mock('./auth/MagicLinkCallback', () => ({ MagicLinkCallback: vi.fn(() => <div data-testid="magic-link-callback" />) }))
vi.mock('./auth/RecoveryScreen', () => ({ RecoveryScreen: vi.fn(() => <div data-testid="recovery-screen" />) }))
vi.mock('./auth/RecoveryPhraseSetup', () => ({
  RecoveryPhraseSetup: vi.fn(() => <div data-testid="recovery-phrase-setup" />),
}))
vi.mock('./devices/PairPrimaryDevice', () => ({
  PairPrimaryDevice: vi.fn(() => <div data-testid="pair-primary-device" />),
}))
vi.mock('./devices/PairNewDevice', () => ({ PairNewDevice: vi.fn(() => <div data-testid="pair-new-device" />) }))
...
describe('router', () => {
  ...
  it('resolves /auth/magic-link and passes baseUrl/token through to MagicLinkCallback', async () => {
    ...
    const { MagicLinkCallback } = await import('./auth/MagicLinkCallback')
  ...
  it('resolves /auth/recover (E2E-only) and passes baseUrl through to RecoveryScreen', async () => {
    ...
    const { RecoveryScreen } = await import('./auth/RecoveryScreen')
  ...
  it('resolves /auth/recovery-phrase-setup (E2E-only), passes every search param through, and onDone is a no-op', async () => {
    ...
    const { RecoveryPhraseSetup } = await import('./auth/RecoveryPhraseSetup')
  ...
  it('resolves /devices/pair-primary (E2E-only) and its getKekForTransfer decodes the kek query param', async () => {
    ...
    const { PairPrimaryDevice } = await import('./devices/PairPrimaryDevice')
  ...
  it('resolves /devices/pair-new (E2E-only); decode rejects and onKekAdopted no-ops when the E2E window hooks are not installed', async () => {
    ...
    const { PairNewDevice } = await import('./devices/PairNewDevice')
