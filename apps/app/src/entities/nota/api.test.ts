...
describe('obterAssinatura', () => {
  ...
  it('GETs the signature with a bearer token and base64-decodes it to Uint8Array', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
  ...
  it('returns { ok: false, code, params } parsed from problem+json on 404 (notes.signature_not_found)', async () => {
    const problem = {
      type: 'about:blank',
