...
import { FilaAssinatura, type ItemFila } from '../../features/nota-fila/FilaAssinatura'
import { EditorSoap } from '../../features/nota-editor/EditorSoap'

export interface FilaEEditorProps {
  itens: readonly ItemFila[]
  /** Chave = `ItemFila.id`. A fila real (fatia 4, backend) substitui isto por um fetch. */
 ...
 */
export function FilaEEditor({ itens, notas, onChangeNota, aoTocar, aoAssinar }: FilaEEditorProps) {
  const { t } = useLingui()
  const [selecionadoId, setSelecionadoId] = useState<string | null>(itens[0]?.id ?? null)
  const notaSelecionada = selecionadoId !== null ? notas[selecionadoId] : undefined

  return (
    <div className="flex">
      <AdaptivePanel label={t`Fila de assinatura`}>
        <FilaAssinatura itens={itens} selecionadoId={selecionadoId} onSelecionar={setSelecionadoId} />
      </AdaptivePanel>
